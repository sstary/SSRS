...
def test(test_ids, all=False, stride=WINDOW_SIZE[0], batch_size=BATCH_SIZE, window_size=WINDOW_SIZE):
    ...
    for img, gt, gt_e in tqdm(zip(test_images, test_labels, eroded_labels), total=len(test_ids), leave=False):
        ...
        for i, coords in enumerate(
            ...
            plt.gca().add_patch(plt.Rectangle((x_comp - 2, y_comp - 2), 2, 2, color='red', fill=False, linewidth=1))
            plt.axis('off')
            plt.savefig('AdasegNet_building.pdf', dpi=1200)
            plt.show()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (images, labels, images_t, labels_t) in enumerate(train_loader):
            ...
            if iter % 100 == 0:
                ...
                start_time = time.time()
                if MIoU > MIoU_best:
                    torch.save(net.state_dict(), './Train_Model/AdasegNet_PIRRG2VIRRG_epoch{}_{}'.format(epoch, MIoU))
                    MIoU_best = MIoU
    print("Train Done!!")
...
for p, id_ in zip(all_preds, test_ids_V):
   ...
   io.imsave('./Test_Vision/AdasegNet_PIRRG2VIRRG_tile_{}.png'.format(id_), img)
