...
FOLDER = "/mnt/nas/xianping/ISPRS_dataset/" # Replace with your "/path/to/the/ISPRS/dataset/folder/"
...
def train(epochs, weights=WEIGHTS, save_epoch=2):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (images, labels, images_t, labels_t) in enumerate(train_loader):
            ...
            pred_target1, pred_target2 = net(images_t)
            ...
            if iter % 1 == 0:
                ...
                print('Train (epoch {}/{}) [{}/{} ({:.0f}%)] lr: {:.12f} lr_D: {:.12f} Loss: {:.12f} Loss_D1: {:.12f} Loss_D2: {:.12f} Accuracy: {:.2f}% Timeuse: {:.2f}'.format(
                    ...
                    loss.data, loss_D1.data, loss_D2.data, accuracy(pred, gt), end_time - start_time))
                start_time = time.time()
            iter += 1
            ...
            if iter % 100 == 0:
                ...
                start_time = time.time()
                MIoU = test(test_ids_V, all=False, stride=256)
                end_time = time.time()
                print('Test Stide_256 time use: ', end_time - start_time)
                start_time = time.time()
                if MIoU > MIoU_best:
                    torch.save(net.state_dict(), './Train_Model/AdasegNet_VIRRG2PRGB_stride_256_epoch{}_{}'.format(epoch, MIoU))
                    MIoU_best = MIoU
    print("Train Done!!")
...
for p, id_ in zip(all_preds, test_ids_V):
   ...
   io.imsave('./Test_Vision/AdasegNet_VIRRG2PRGB_3975_tile_{}.png'.format(id_), img)
